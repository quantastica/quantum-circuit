	...
	if(name == "measure") {
		...
		if(qreg && creg) {
			...
			if(qbit < 0 && cbit < 0) {
				...
				for(var x = 0; x < numBits; x++) {
					self.circuit.addGate("measure", tmpCol, x + self.qregBase(qreg), { creg: { name: creg, bit: x }, condition: condition || {} } );
				}
				...
				if(qbit >= 0 && cbit >= 0) {
					...
					if(qbit >= 0 && cbit < 0) {
						...
						for(var x = 0; x < numBits; x++) {
							self.circuit.addGate("measure", tmpCol, x + self.qregBase(qreg), { creg: { name: creg, bit: cbit }, condition: condition || {} } );
						}						
