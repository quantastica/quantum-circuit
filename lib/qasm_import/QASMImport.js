...
QCQASMListener.prototype.enterStatement = function(ctx) {
	...
	if(ctx.gatedecl()) {
					...
					if(explist) {
						...
						if(gateDef) {
							...
							for(var i = 0; i < paramCount; i++) {
								var paramValue = explist.getChild(i).getText();
								if(paramValue != ",") {
									var paramName = paramDef.length >= counter ? paramDef[counter] : "";
									if(paramName) {
		...
		var customDecomposed = customGate.save(false);
		...
		if(qreg && creg) {
			...
			if(qbit < 0 && cbit < 0) {
				...
				for(var x = 0; x < numBits; x++) {
					tmpWires.push(x + self.qregBase(qreg));
				}
				var tmpCol = self.circuit.lastNonEmptyPlace(tmpWires) + 1;

				for(var x = 0; x < numBits; x++) {
					self.circuit.addGate("measure", tmpCol, x + self.qregBase(qreg), { creg: { name: creg, bit: x } } );
				}
				...
				if(qbit >= 0 && cbit >= 0) {
					...
					//
					self.circuit.addGate("measure", -1, qbit + self.qregBase(qreg), { creg: { name: creg, bit: cbit } } );
				} else {
					if(qbit >= 0 && cbit < 0) {
						//
						...
						//
						for(var x = 0; x < numBits; x++) {
							self.circuit.addGate("measure", -1, qbit + self.qregBase(qreg), { creg: { name: creg, bit: x } } );
						}						
					} else {
						//
						...
						for(var x = 0; x < numBits; x++) {
							self.circuit.addGate("measure", tmpCol, x + self.qregBase(qreg), { creg: { name: creg, bit: cbit } } );
						}						
