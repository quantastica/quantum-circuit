...
var convertRelativeLinksToAbsolute = function (s, baseURL) {
	...
	links.map(function (link) {
		...
		if (
			...
			newlink = link.full.replace(
				/\((.+?)\)/g,
				"(" + url.resolve(baseURL, link.link) + ")",
			);
		}
...
fs.readFile(path.join(__dirname, "README_TEMPLATE.md"), function (err, data) {
	...
	for (var gateName in circ.basicGates) {
		...
		var numQubits = Math.log2(
			gateDef.matrix && gateDef.matrix.length ? gateDef.matrix.length : 2,
		);
		var pyquilDef =
			(gateDef.exportInfo
				? gateDef.exportInfo.pyquil || gateDef.exportInfo.quil
				: null) || null;
		var cirqDef = (gateDef.exportInfo ? gateDef.exportInfo.cirq : null) || null;
		var qsharpDef =
			(gateDef.exportInfo ? gateDef.exportInfo.qsharp : null) || null;
		...
		if (pyquilDef && pyquilDef.replacement) {
			...
			if (gdef) {
				var pdef =
					(gdef.exportInfo
						? gdef.exportInfo.pyquil || gdef.exportInfo.quil
						: null) || null;
				if (pdef) {
	...
	readme = readme.replace(
		new RegExp(escapeRegExp("{TITLE}"), "g"),
		"# Quantum Circuit Simulator",
	);
	readme = readme.replace(
		new RegExp(escapeRegExp("{GATE_INDEX}"), "g"),
		gateIndex,
	);
	readme = readme.replace(
		new RegExp(escapeRegExp("{GATE_REFERENCE}"), "g"),
		gateRef,
	);
	readme = readme.replace(new RegExp(escapeRegExp("{API_DOCS}"), "g"), apiDocs);

	fs.writeFile(
		path.join(path.join(__dirname, "../"), "README.md"),
		readme,
		function (err) {
			if (err) {
				throw err;
